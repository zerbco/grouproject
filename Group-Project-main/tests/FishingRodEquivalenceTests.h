...
private:
    ...
    void testLowRarityRod() {
        FishingRod rod(3, 1.5f, 1, 0, "test", "test");
        Fish* fish = rod.bite();
        if (fish) {
    ...
    void testMediumRarityRod() {
        FishingRod rod(3, 1.5f, 5, 0, "test", "test");
        Fish* fish = rod.bite();
        if (fish) {
    ...
    void testHighRarityRod() {
        FishingRod rod(3, 1.5f, 10, 0, "test", "test");
        Fish* fish = rod.bite();
        if (fish) {
