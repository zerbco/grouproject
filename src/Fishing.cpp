...
Fishing::Fishing(FishingInfo _info, sf::Vector2u _position, FishingRod *rod): fish_info(_info), position(_position), capture_percentage(0), rod(rod){
...
void Fishing::updateFish(Randomizer *rd){
        if (state == 1){
...
void Fishing::update(Randomizer *rd){
    updateFish(rd);
    updatePhysics();
    updatePositions();
    updateProgress();
